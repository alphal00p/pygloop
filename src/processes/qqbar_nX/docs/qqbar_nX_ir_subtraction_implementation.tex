...
loop-energy poles in the graph-global numerator.

The current CFF-compatible auxiliary denominator mode is
\texttt{spatial\_proxy}.  It removes the opposite gluon propagator from the CT
topology and keeps the leading fixed-\(\zeta\) auxiliary denominator as a
loop-energy-free graph numerator factor:
\begin{align}
  B_1(G)
  &=
  -2(p_0\cdot p_1)
...
\end{align}
This is the most important current engineering choice: denominators with
actual loop-energy dependence are part of the graph topology; graph-global
inverse factors are allowed only when they are purely spatial.
...
auxiliary denominator built from a fixed vector.  The \qqbarNX{} code uses the
same local leading-collinear replacement logic for the two ISR regions, but
differs in three implementation-level ways:
\begin{enumerate}[leftmargin=2em]
  \item the heavy subgraph is a top pentagon with three Higgs insertions;
  \item no UV subtraction operator is generated;
  \item the current CFF-compatible \DOT{} writes the auxiliary denominator in
        the spatial-proxy form of Eq.~\eqref{eq:spatialProxy}, not as a
        graph-global denominator with loop-energy dependence.
\end{enumerate}
