...
class VacuumDotGraph(object):
    ...
    def compute_directed_winding_from_cut(self, directed_cycle: Set[pydot.Edge], cut: List[pydot.Edge]) -> int:
    ...
    def cut_splits_into_two_components(self, initial_cut: List[pydot.Edge], final_cut: List[pydot.Edge]) -> bool:
    ...
    def route_cut_graph(self, graph: pydot.Dot, initial_cut: List[pydot.Edge], final_cut: List[pydot.Edge], partition: List[List[str]]):
    ...
    def check_routing(self, graph: pydot.Dot, partition: List[List[str]]) -> bool:
        edges = graph.get_edges()
        nodes = []
        for e in edges:
            nodes.append(_node_key(e.get_source()))
    ...
    def phase_space_check(self, initial_cut: List[pydot.Edge], final_cut: List[pydot.Edge]) -> bool:
        if len(set(initial_cut)-set(final_cut))==1:
            return False
        return True

    # Computes all possible cut routed diagrams given a process definition
    def cut_graphs_with_routing(self, initial_massive: List[str], final_massive: List[str],) -> List[Tuple[List[pydot.Edge], List[pydot.Edge], List[List[pydot.Edge]], pydot.Dot]]:
        ...
        for initial_cut in initial_cuts:
            for final_cut in final_cuts:
                connected_components = self.cut_splits_into_two_components(initial_cut, final_cut)
                if self.phase_space_check(initial_cut,final_cut) and connected_components and self.set_cut_labels(initial_cut, final_cut, copy.deepcopy(self.dot), cycles)!=False:

                    graph = self.set_cut_labels(initial_cut, final_cut, copy.deepcopy(self.dot), cycles) #self.set_cut_labels(initial_cut, final_cut, connected_components)(initial_cut, final_cut, copy.deepcopy(self.dot), cycles)
    ...
    def cut_graphs_with_routing_leading_virtuality(self, initial_massive: List[str], final_massive: List[str])-> List[Tuple[List[pydot.Edge], List[pydot.Edge], List[List[pydot.Edge]], pydot.Dot]]:
        ...
        for graph in cut_graphs_with_routing:
            ...
            for e in graph[3].get_edges():
                if sp.Rational(e.get_attributes()["routing_p1"])!=0 and sp.Rational(e.get_attributes()["routing_k0"])==0 and sp.Rational(e.get_attributes()["routing_p2"])==0 and _strip_quotes(e.get_attributes().get("particle", "")) != "a":
                    count_p1=True
                elif sp.Rational(e.get_attributes()["routing_p2"])!=0 and sp.Rational(e.get_attributes()["routing_k0"])==0 and sp.Rational(e.get_attributes()["routing_p1"])==0 and _strip_quotes(e.get_attributes().get("particle", "")) != "a":
                    count_p2=True
...
class DYDotGraph(DotGraph):
        ...
        for e in incoming_edges:
            for ep in outgoing_edges:
                if _edge_particle(e) == _edge_particle(ep) and ep not in paired_up:
                    vacuum_graph.add_edge(remove_edge_attr(self.edge_fusion(e, ep), "lmb_rep"))
                    paired_up.append(ep)
    ...
    def contains_ext(self, node_subset, left_externals, right_externals):
        ...
        for e in self.dot.get_edges():
            if _base_node(e.get_source()) in node_subset and _base_node(e.get_destination()) in node_subset:
    ...
    def enumerate_cutkosky_cuts(self, left_externals, right_externals):
        ...
        good_sets = [
            S for S in all_cuts if is_connected(self.dot, S) and is_connected(self.dot, nodes - S) and self.contains_ext(S, left_externals, right_externals)
        ]
