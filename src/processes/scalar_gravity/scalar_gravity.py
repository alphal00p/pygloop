...
class ScalarGravity(object):
    ...
    def set_model(self) -> None:
        ...
        self.gl_worker.run(
            "set model mass_scalar_1={{re:{:.16f},im:0.0}}".format(self.m_bh1)
        )
        self.gl_worker.run(
            "set model mass_scalar_2={{re:{:.16f},im:0.0}}".format(self.m_bh2)
        )
        ...
        match self.n_loops:
            case 1:
                ...
                input_dot_graphs = pjoin(
                    RESOURCES_FOLDER, self.name_for_resources, "1L_graphs.dot"
                )
                self.gl_worker.run(
                    f"import graphs {input_dot_graphs} -p {base_name} -i {graphs_process_name}"
                )
                self.gl_worker.run("save state -o")
                dot_files_1L = self.gl_worker.get_dot_files(
                ...
                dot_files_1L_processed = classical_limit_processor.process_graphs(
                    dot_files_1L
                )
                dot_files_1L_processed.save_to_file(
                    pjoin(DOTS_FOLDER, self.name, f"{integrand_name}.dot")
                )
            case 2:
                logger.info("Importing two-loop graphs ...")
                input_dot_graphs = pjoin(
                    RESOURCES_FOLDER, self.name_for_resources, "2L_graphs.dot"
                )
                ...
                self.gl_worker.run("save state -o")
                dot_files_2L = self.gl_worker.get_dot_files(
                    process_id=None,
                ...
                dot_files_2L_processed = classical_limit_processor.process_graphs(
                    dot_files_2L
                )
    ...
    def valide_ps_point(self) -> None:
        ...
        for i_p, p in enumerate(self.ps_point[:2]):
            ...
            if abs(m_bh - target_m) / sqrt_s > TOLERANCE:
                raise pygloopException(
                    f"Incoming black hole #{i_p + 1} must be onshell."
                )
        for i_p, p in enumerate(self.ps_point[2:]):
            m_bh = math.sqrt(abs(p.squared()))
            p_sum -= p
            ...
            if abs(m_bh - target_m) / sqrt_s > TOLERANCE:
                raise pygloopException(
                    f"Outgoing black hole #{i_p + 1} must be onshell."
                )
    ...
    def integrand_xspace(
        ...
        try:
            if multi_channeling is False:
                ...
                raise pygloopException(
                    "Multi-channeling not implemented for the scalar gravity processes."
                )
    ...
    def spenso_integrand(
        self, loop_momenta: list[Vector], parametric=False, prefer_eager_mode=False
    ) -> complex:
        try:
