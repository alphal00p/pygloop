...
class GGHHH(object):
    ...
    DEBUG_FULL_EVALUATOR_PATH = (
        None  # "/Users/vjhirsch/Documents/Work/pygloop/TMP/cff_evaluator_inputs.py"
    )
    ...
    def __init__(
        ...
        **opts,
        ...
        if toml_config_path is None:
            toml_config_path = pjoin(
                CONFIGS_FOLDER, self.name_for_resources, "generate.toml"
            )
        ...
        if runtime_toml_config_path is None:
            runtime_toml_config_path = pjoin(
                CONFIGS_FOLDER, self.name_for_resources, "runtime.toml"
            )
        ...
        for output_name, output_id in amplitudes.items():
            ...
            if not os.path.isdir(
                pjoin(
                    ...
                    "processes",
                    "amplitudes",
                    self.get_integrand_name(suffix=""),
                    output_name,
                    "integrand",
                )
            ):
        ...
        for integrand_name in [ self.get_integrand_name(), ]:  # fmt: off
            integrand_evaluator_directory = pjoin(
                EVALUATORS_FOLDER, self.name, integrand_name
            )
            ...
            for evaluator_name in GGHHH.SPENSO_EVALUATOR_NAMES:
                if os.path.exists(
                    pjoin(integrand_evaluator_directory, f"{evaluator_name}.so")
                ):
                    at_least_one_evaluator_found = True
                    evaluators[evaluator_name] = PygloopEvaluator.load(
                        pjoin(EVALUATORS_FOLDER, self.name, integrand_name),
                        evaluator_name,
                    )
                    if evaluator_name == "input_parameters_evaluator":
                        parameters_param_builders.append(
                            evaluators[evaluator_name].param_builder
                        )
                    else:
                        integrand_param_builders.append(
                            evaluators[evaluator_name].param_builder
                        )
                else:
            ...
            if at_least_one_evaluator_found:
                ...
                self.initialize_param_builders(
                    integrand_param_builders, parameters_param_builders[0]
                )
            else:
                self.spenso_evaluators[integrand_name] = None
    ...
    def get_model(self):
        try:
            ...
            raise pygloopException(
                f"Could not get model from GammaLoop worker. Error: {e}"
            ) from e
    ...
    def generate_graphs(self) -> None:
        ...
        match self.n_loops:
            case 1:
                ...
                self.gl_worker.run("save state -o")
                GGHHH_1L_dot_files = self.gl_worker.get_dot_files(
                ...
                GGHHH_1L_dot_files_processed = self.process_1L_generated_graphs(
                    DotGraphs(dot_str=GGHHH_1L_dot_files)
                )
                GGHHH_1L_dot_files_processed.save_to_file(
            ...
            case 2:
                ...
                self.gl_worker.run("save state -o")
                GGHHH_2L_dot_files = self.gl_worker.get_dot_files(
                    process_id=None, integrand_name=graphs_process_name
                )
                write_text_with_dirs(
                    pjoin(DOTS_FOLDER, self.name, f"{graphs_process_name}.dot"),
                ...
                self.gl_worker.run("save dot")
                GGHHH_2L_dot_files_processed = self.process_2L_generated_graphs(
                    DotGraphs(dot_str=GGHHH_2L_dot_files)
                )
                GGHHH_2L_dot_files_processed.save_to_file(
                    pjoin(DOTS_FOLDER, self.name, f"{integrand_name}.dot")
                )
            case 3:
                logger.info("Importing three-loop graphs ...")
                three_loop_input_dot_graphs = pjoin(
                    RESOURCES_FOLDER, self.name_for_resources, "3L_graphs.dot"
                )
                self.gl_worker.run(
                    f"import graphs {three_loop_input_dot_graphs} -p {base_name} -i {graphs_process_name}"
                )
                self.gl_worker.run("save state -o")
                GGHHH_3L_dot_files = self.gl_worker.get_dot_files(
                    process_id=None, integrand_name=graphs_process_name
                )
                write_text_with_dirs(
                    pjoin(DOTS_FOLDER, self.name, f"{graphs_process_name}.dot"),
                ...
                self.gl_worker.run("save dot")
                GGHHH_3L_dot_files_processed = self.process_3L_generated_graphs(
                    DotGraphs(dot_str=GGHHH_3L_dot_files)
                )
                GGHHH_3L_dot_files_processed.save_to_file(
                    pjoin(DOTS_FOLDER, self.name, f"{integrand_name}.dot")
                )
            case _:
        ...
        if graphs_process_name not in amplitudes:
            raise pygloopException(
                f"Amplitude with named integrand {graphs_process_name} not found in GammaLoop state. Generate graphs first."
            )
    ...
    def initialize_param_builders(
        ...
        for i_p, p in enumerate(self.ps_point):
            input_params_param_builder.set_parameter_values(
                (self.SB["externalP"], E(str(i_p))),
                [complex(p_i) for p_i in p.to_list()],
            )

        # Add the polarization vectors
        for e_i in [0, 1]:
            pol_vector = vxxxxx(
                self.ps_point[e_i].to_list(), 0.0, self.helicities[e_i], -1
            )[2::]
            for itg_pb in integrand_param_builders:
                itg_pb.set_parameter_values(
                    (self.SB["vector_pol"], E(str(e_i))),
                    [complex(pol_i) for pol_i in pol_vector],
                )
    ...
    def set_from_sample(
        ...
        if ks is not None:
            ...
            for i_k, k in enumerate(ks):
                input_params_evaluator.param_builder.set_parameter_values(
                    (self.SB["Kspatial"], E(str(i_k))),
                    [complex(k_i) for k_i in k.to_list()],
                )

            # Evaluate derived inputs
            derived_inputs = input_params_evaluator.evaluate()
            integrand_param_builder.set_parameter_values_within_range(
                0, len(derived_inputs), derived_inputs
            )

        # Add the E-surfaces selectors
        if cff_term is not None and family_id is not None:
            integrand_param_builder.set_parameter_values(
                (self.SB["etaSigma"], self.SB["o_id"], self.SB["f_id"]),
                cff_term.masks[family_id],
            )

        # Add the energy signs
        if cff_term is not None:
            integrand_param_builder.set_parameter_values(
                (self.SB["energySign"], self.SB["o_id"]), cff_term.orientation_signs
            )

    def build_parametric_integrand_evaluator(
        self, graph: DotGraph, integrand: Expression, cff_structure: CFFStructure
    ) -> PygloopEvaluator:
        internal_edges = graph.get_internal_edges()
        max_internal_edge_id = (
            0
            if len(internal_edges) == 0
            else max(int(e.get("id")) for e in internal_edges)
        )
        ...
        param_builder.add_parameter_list(
            (CFFStructure.SB["E"],), max_internal_edge_id + 1
        )
        ...
        for e_i in [0, 1]:
            ...
            param_builder.force_parameters_to_complex([
                (self.SB["vector_pol"], E(str(e_i)))
            ])
        # Add the E-surfaces selectors
        param_builder.add_parameter_list(
            (self.SB["etaSigma"], self.SB["o_id"], self.SB["f_id"]),
            len(cff_structure.e_surfaces),
        )
        # Add the energy signs
        param_builder.add_parameter_list(
            (self.SB["energySign"], self.SB["o_id"]), max_internal_edge_id + 1
        )
    ...
    def build_full_integrand_evaluator(
        ...
        if strategy not in ["merging", "summing", "function_map"]:
            raise pygloopException(
                f"Evaluation combination strategy '{strategy}' not supported."
            )
        total_start = time.time()

        selector_execution = [
            Replacement(
                self.SB["etaSelector"](E("1"), E("true_"), E("false_")), E("true_")
            ),
            Replacement(
                self.SB["etaSelector"](E("0"), E("true_"), E("false_")), E("false_")
            ),
        ]
        ...
        total_evaluators_to_build = sum(
            len(term.families) for term in cff_structure.expressions
        )
        ...
        progress_format = progressbar.FormatCustomText(
            progress_format_str, progress_format_mapping
        )
        ...
        for o_i in range(n_edge_orientations):
            parametric_integrand_arguments.append(
                self.SB["parametricEnergySign"](E(str(o_i)))
            )
        for eta_i in range(n_e_surfaces):
            parametric_integrand_arguments.append(
                self.SB["parametricEtaSigma"](E(str(eta_i)))
            )
        parametric_integrand_function_signature = self.SB["ICFF"](
            *parametric_integrand_arguments
        )
        # fmt: off
        ...
        for cff_i, cff_term in enumerate(cff_structure.expressions):
            ...
            for o_i, o in enumerate(cff_term.orientation):
                if strategy == "function_map":
                    orientation_substitutions.append(
                        Replacement(
                            self.SB["parametricEnergySign"](E(str(o_i))),
                            E("-1") if o.is_reversed() else E("1"),
                        )
                    )
                else:
                    orientation_substitutions.append(
                        Replacement(
                            self.SB["energySign"](self.SB["o_id"], E(str(o_i))),
                            E("-1") if o.is_reversed() else E("1"),
                        )
                    )
            if strategy == "function_map":
            ...
            orientation_substituted_integrand = parametric_expression.replace_multiple(
                orientation_substitutions
            )
            ...
            for family_i, f in enumerate(cff_term.families):
                if progress_bar is not None:
                    merge_avg = (
                        (total_merge_time / merge_calls) if merge_calls > 0 else 0.0
                    )
                    eval_avg = (
                        (total_evaluator_time / evaluator_calls)
                    ...
                    progress_format.update_mapping(**{
                        ...
                        **(
                            {"merge_avg": merge_avg, "eval_avg": eval_avg}
                            if strategy == "merging"
                            else {}
                        ),
                    })
                ...
                for eta_i, is_present in enumerate(f):
                    if strategy == "function_map":
                        ...
                        e_surface_selector_substitutions.append(
                            Replacement(
                                self.SB["etaSigma"](
                                    self.SB["o_id"], self.SB["f_id"], E(str(eta_i))
                                ),
                                E("1") if is_present else E("0"),
                            )
                        )
                concretized_integrand = (
                    orientation_substituted_integrand.replace_multiple(
                        e_surface_selector_substitutions
                    )
                )
                if strategy != "function_map":
                    concretized_integrand = concretized_integrand.replace_multiple(
                        selector_execution
                    )
                n_terms += 1
                if strategy in ["summing", "function_map"]:
                    ...
                    if GGHHH.COMPLEXIFY_EVALUATOR:
                        raise pygloopException(
                            "Complexification of evaluators deprecated."
                        )
                    ...
                    if full_evaluator is None:
                        ...
                        full_evaluator.merge(
                            concretized_evaluator, cpe_iterations=n_cpe_iterations
                        )
        ...
        if strategy in ["summing", "function_map"]:
            logger.info(
                f"Generating full evaluator with symbolica using strategy '{strategy}' ..."
            )
            if strategy == "function_map":
                ...
                for o_i in range(n_edge_orientations):
                    function_args_replacement.append(
                        Replacement(
                            self.SB["parametricEnergySign"](E(str(o_i))),
                            E(f"{self.SB['parametricEnergySign'].get_name()}_{o_i}"),
                        )
                    )
                for eta_i in range(n_e_surfaces):
                    function_args_replacement.append(
                        Replacement(
                            self.SB["parametricEtaSigma"](E(str(eta_i))),
                            E(f"{self.SB['parametricEtaSigma'].get_name()}_{eta_i}"),
                        )
                    )
                parametric_integrand_arguments = [
                    arg.replace_multiple(function_args_replacement)
                    for arg in parametric_integrand_arguments
                ]
                parametric_integrand_function_definition = (
                    parametric_integrand_function_definition.replace_multiple(
                        function_args_replacement
                    )
                )
                functions: dict[
                    tuple[Expression, str, tuple[Expression]], Expression
                ] = {
                    (
                        self.SB["ICFF"],
                        "ICFF",
                        tuple(parametric_integrand_arguments),
                    ): parametric_integrand_function_definition,
                }
                conditionals = [self.SB["etaSelector"]]
            else:
            ...
            if GGHHH.DEBUG_FULL_EVALUATOR_PATH is not None:
                logger.critical(
                    "Debugging full evaluator generation inputs written to %s",
                    GGHHH.DEBUG_FULL_EVALUATOR_PATH,
                )
                with open(GGHHH.DEBUG_FULL_EVALUATOR_PATH, "w") as debug_file:
                    ...
                    debug_file.write(
                        f"expression=E('{full_expression.to_canonical_string()}')\n"
                    )
                    debug_file.write(
                    ...
                    for k, v in functions.items():
                        args_str = ", ".join(
                            f'E("{arg.to_canonical_string()}")' for arg in k[2]
                        )
                        functions_formatted.append(
                            f'(E("{k[0].to_canonical_string()}"), "{k[1]}", ({args_str})): E("{v.to_canonical_string()}")'
                        )
                    debug_file.write(
                        f"functions={{ {', '.join(functions_formatted)} }}\n"
                    )
                    debug_file.write(
                        f"input_params=[{', '.join(f'E("{param.to_canonical_string()}")' for param in input_params)}]\n"
                    )
                    if conditionals is not None:
                        debug_file.write(
                            f"conditionals=[{', '.join(f'E("{cond.to_canonical_string()}")' for cond in conditionals)}]\n"
                        )
                    else:
                        debug_file.write("conditionals=None\n")
                    debug_file.write(
                        f"real_components={integrand_param_builder.get_real_components()}\n"
                    )
                    debug_file.write(
                        f"components_phase={integrand_param_builder.get_components_phase()}\n"
                    )
            ...
            if GGHHH.FREEZE_INPUT_PHASES and not GGHHH.COMPLEXIFY_EVALUATOR:
                full_evaluator.set_real_params(  # pyright: ignore
                    integrand_param_builder.get_real_components(),
                    sqrt_real=True,
                    log_real=True,
                    powf_real=True,
        ...
        evaluator_pct = (
            (total_evaluator_time / total_time * 100.0) if total_time > 0 else 0.0
        )
    ...
    def build_parameter_evaluators(
        ...
        max_external_edge_id = (
            0
            if len(external_edges) == 0
            else max(int(e.get("id")) for e in external_edges)
        )  # noqa: F841

        lmb_projections = [
            Replacement(source, target)
            for source, target in graph.get_emr_replacements(
                head=self.SB["Q"].get_name()
            )
        ]
        ...
        for e_i in range(max_internal_edge_id + 1):
            if e_i <= max_external_edge_id:
                ...
                external_energy = external_energy.replace_multiple(
                    lmb_projections, repeat=True
                )
                computable_parameters.append(external_energy)
            else:
                particle = model.get_particle(
                    edges[e_i].get("particle").replace('"', "")
                )
                on_shell_energy = E("0")
                for i in range(1, 4):
                    on_shell_energy += self.SB["Q"](E(f"{e_i}"), E(str(i))) * self.SB[
                        "Q"
                    ](E(f"{e_i}"), E(str(i)))
                if particle.mass.name.upper() != "ZERO":
                    on_shell_energy += S(f"UFO::{particle.mass.name}") ** 2
                on_shell_energy = on_shell_energy ** E("1/2")
                on_shell_energy = on_shell_energy.replace_multiple(
                    lmb_projections, repeat=True
                )
                # Map to Kspatial
                on_shell_energy = on_shell_energy.replace(
                    E("gammalooprs::K(x_,y_)"), self.SB["Kspatial"](E("x_"), E("y_-1"))
                )
        ...
        for e_i in range(max_internal_edge_id + 1):
            for i in range(1, 4):
                ...
                spatial_momentum = spatial_momentum.replace_multiple(
                    lmb_projections, repeat=True
                )
                # Map to Kspatial
                spatial_momentum = spatial_momentum.replace(
                    E("gammalooprs::K(x_,y_)"), self.SB["Kspatial"](E("x_"), E("y_-1"))
                )
        ...
        pygloop_evaluator = PygloopEvaluator(
            evaluator,
            param_builder,
            "input_parameters_evaluator",
            output_length=len(computable_parameters),
        )
        if GGHHH.FREEZE_INPUT_PHASES and not GGHHH.COMPLEXIFY_EVALUATOR:
    ...
    def generate_spenso_code(
        ...
        **opts,
        ...
        if full_spenso_integrand_strategy not in [
            None,
            "merging",
            "summing",
            "function_map",
        ...
        spenso_evaluator_compilation_options.update(
            evaluators_compilation_options or {}
        )
        ...
        gghhh_dot_graphs = DotGraphs(
            dot_str=self.gl_worker.get_dot_files(
                process_id=process_id, integrand_name=integrand_name
            )
        )
        ...
        numerator = simplify_color(
            gghhh_graph.get_numerator() * gghhh_graph.get_projector()
        )
        ...
        numerator_expr = numerator_expr.replace(
            E("spenso::cind(x_)"), E("x_"), repeat=True
        )

        # Now substitute with on-shell energies, and rename spatial components
        numerator_expr = numerator_expr.replace(
            E("gammalooprs::Q(x_,0)"),
            self.SB["energySign"](self.SB["o_id"], E("x_"))
            * CFFStructure.SB["E"](S("x_")),
            repeat=True,
        )
        numerator_expr = numerator_expr.replace(
            E("gammalooprs::Q(x_,i_)"),
            self.SB["Qspatial"](S("x_"), S("i_") - 1),
            repeat=True,
        )
        ...
        for e_surf in cff_structure.e_surfaces:
            cff_term *= self.SB["etaSelector"](
                self.SB["etaSigma"](self.SB["o_id"], self.SB["f_id"], e_surf.id),
                e_surf.expression,
                1,
            )
        ...
        parametric_evaluator = self.build_parametric_integrand_evaluator(
            gghhh_graph, integrand_expression, cff_structure
        )
        ...
        params_evaluator = self.build_parameter_evaluators(
            gghhh_graph,
            n_hornerscheme_iterations=n_hornerscheme_iterations,
            _n_cpe_iterations=n_cpe_iterations,
        )
        self.initialize_param_builders(
        ...
        evaluator_directory = pjoin(
            EVALUATORS_FOLDER, self.name, self.get_integrand_name()
        )
        os.makedirs(evaluator_directory, exist_ok=True)

        logger.info(
            f"Compiling evaluators in {Colour.GREEN}{evaluator_directory}{Colour.END}..."
        )
        t_compile_start = time.time()
        parametric_evaluator.compile(
            evaluator_directory,
            integrand_evaluator_compiler=integrand_evaluator_compiler,
        ...
        size_on_disk = self.get_size_on_disk(
            {
                "integrand_type": "spenso_parametric",
                "evaluator_compilation": "symbolica",
            }
            if full_spenso_integrand_strategy is None
            else {
                "integrand_type": "spenso_summed",
                "evaluator_compilation": "symbolica",
            }
        )
        if size_on_disk is None:
            ...
            size_on_disk_str = (
                f"{Colour.GREEN}{size_on_disk / 1000_000.0:.2f} MB{Colour.END}"
            )
        logger.info(
        ...
        logger.info(
            f"Saved spenso integrand evaluators to {Colour.GREEN}{evaluator_directory}{Colour.END}"
        )
        ...
        if GGHHH.DEBUG_FULL_EVALUATOR_PATH is not None:
            ...
            logger.critical(
                "Debugging full evaluator call inputs written to %s",
                GGHHH.DEBUG_FULL_EVALUATOR_PATH,
            )
    ...
    def generate_gammaloop_code(self) -> None:
        ...
        if os.path.isfile(
            pjoin(
                ...
                "gammaloop_states",
                self.name,
                f"{integrand_name}_is_generated",
            )
        ):
        ...
        with open(
            pjoin(
                ...
                "gammaloop_states",
                self.name,
                f"{integrand_name}_is_generated.txt",
            ),
            "w",
        ) as f:
            f.write("generated")
        ...
        try:
            if multi_channeling is False:
                ...
                if len(xs) != expected_len:
                    raise pygloopException(
                        f"Expected {expected_len} integration variables for {n_loops} loop momenta, received {len(xs)}."
                    )
                loop_momenta: list[Vector] = []
                total_jac = 1.0
                for i_loop in range(n_loops):
                    k, jac = self.parameterize(
                        xs[3 * i_loop : 3 * (i_loop + 1)], parameterization
                    )
        ...
        try:
            match integrand_implementation["integrand_type"]:
                case "spenso_parametric":
                    return self.spenso_integrand(
                        loop_momenta, integrand_implementation, parametric=True
                    )
                case "spenso_summed":
                    return self.spenso_integrand(
                        loop_momenta, integrand_implementation, parametric=False
                    )
                case "gammaloop":
    ...
    def spenso_integrand(
        ...
        if not parametric:
            ...
            full_evaluator_result = full_evaluator.evaluate(
                eager=prefer_eager_mode,
                prefer_symjit=(
                    integrand_implementation["evaluator_compiler"] == "symjit"
                ),
            )
            ...
            for cff_term in cff_structure.expressions:
                for f_i in range(len(cff_term.families)):
                    self.set_from_sample(
                        parametric_evaluator, cff_term=cff_term, family_id=f_i
                    )
    ...
    def gammaloop_integrator(
        ...
        **opts,
        ...
        if run_workspace_name is None:
            workspace_dir = pjoin(
                INTEGRATION_WORKSPACE_FOLDER, self.name, integrand_name
            )
        else:
            workspace_dir = pjoin(
                INTEGRATION_WORKSPACE_FOLDER, self.name, run_workspace_name
            )
    ...
    def get_size_on_disk(self, integrand_implementation: dict[str, Any]) -> int | None:
        ...
        match integrand_implementation["integrand_type"]:
            case "spenso_parametric":
                ...
                size = 0
                if (
                    integrand_implementation.get("evaluator_compilation", "symbolica")
                    == "symjit"
                ):
                    files_list = [
            ...
            case "spenso_summed":
                ...
                if (
                    ...
                    files_list = [
                        "input_parameters_evaluator.so",
                        "full_integrand_evaluator.sjb",
                    ]
                else:
                    files_list = [
                        "input_parameters_evaluator.so",
                        "full_integrand_evaluator.so",
                    ]
                for file_name in files_list:
                    file_path = pjoin(evaluator_directory, file_name)
                    if os.path.exists(file_path):
    ...
    def naive_integrator(
        ...
        **opts,
        ...
        return run_naive_integrator(
            self, parameterisation, integrand_implementation, target, **opts
        )
    ...
    def vegas_integrator(
        ...
        **opts,
        ...
        return run_vegas_integrator(
            self, parameterisation, integrand_implementation, _target, **opts
        )
    ...
    def symbolica_integrator(
        ...
        **opts,
        ...
        return run_symbolica_integrator(
            self, parameterisation, integrand_implementation, target, **opts
        )
